...
bordersmote = SMOTE()
X_resampled, y_resampled = bordersmote.fit_resample(cleandata, df_y)
X_resampled.shape
y_resampled.shape
...
data = data.drop(['Date', 'Machine_ID'], axis = 1) # Excluding id column
data.info()
cleandata = pd.DataFrame(imp_scale.transform(data), columns = imp_scale.get_feature_names_out())
columns_list = list(cleandata.columns)
